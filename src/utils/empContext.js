...
export const EmpProvider = ({ children }) => {
  ...
  useEffect(() => {
    fetch("http://158.220.96.121:8080/api/empauth/session", {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const login = async (credentials) => {
    const res = await fetch("http://158.220.96.121:8080/api/empauth/login", {
      method: "POST",
      credentials: "include",
  ...
  const logout = async () => {
    await fetch("http://158.220.96.121:8080/api/empauth/logout", {
      method: "POST",
      credentials: "include",
    });
    setEmployee(null);
