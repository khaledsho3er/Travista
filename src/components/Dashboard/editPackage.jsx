...
const EditPackage = ({
  ...
  useEffect(() => {
    if (packageData) {
      ...
      // Fixed flights initialization - properly separate fields
      setFlights(
        packageData.flights && packageData.flights.length > 0
          ? packageData.flights.map((flight) => ({
              airline: flight.airline || "",
              date: flight.date || "",
              from: flight.from || flight.route || "", // Handle legacy route field
              to: flight.to || "",
              departDate: flight.departDate || flight.depart || "",
              departTime: flight.departTime || "",
              arrivalDate: flight.arrivalDate || flight.arrival || "",
      ...
      // Fixed hotels initialization - ensure city objects are properly handled
      setHotels(
        packageData.hotels && packageData.hotels.length > 0
          ? packageData.hotels.map((hotel) => {
              // Find the city object from the cities list if it's just a string
              let cityObj = null;
              if (typeof hotel.city === "string") {
                // This will be handled once cities are loaded
                cityObj = { name: hotel.city };
              } else if (hotel.city && hotel.city.name) {
                cityObj = hotel.city;
              }
  ...
  // Effect to update hotel cities once cities are loaded
  useEffect(() => {
    if (cities.length > 0 && packageData?.hotels) {
      setHotels((prevHotels) =>
        prevHotels.map((hotel, index) => {
          if (
            packageData.hotels[index] &&
            typeof packageData.hotels[index].city === "string"
          ) {
            const cityObj = cities.find(
              (city) => city.name === packageData.hotels[index].city
            );
            return {
              ...hotel,
              city: cityObj || { name: packageData.hotels[index].city },
            };
          }
          return hotel;
        })
  ...
  const handleSubmit = async () => {
    try {
      ...
      const updatedPackageData = {
        ...
        // Fixed flights array with proper field mapping
        flights: flights
          .filter((flight) => flight.airline.trim() !== "")
          .map((flight) => ({
            airline: flight.airline,
            date: flight.date,
            from: flight.from,
            ...
            <Stack spacing={2} key={index}>
              ...
              <Stack direction="row" spacing={2}>
                <TextField
                  ...
                  onChange={(e) => {
                    const newFlights = [...flights];
                    newFlights[index].departDate = e.target.value;
                    setFlights(newFlights);
                  }}
                />
                <TextField
                  ...
                  onChange={(e) => {
                    const newFlights = [...flights];
                    newFlights[index].departTime = e.target.value;
                    setFlights(newFlights);
                  }}
                />
              </Stack>
